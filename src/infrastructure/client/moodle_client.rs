...
impl MoodleClient {
    pub fn new(base_url: String, format: String) -> Self {
        Self { client: Client::new(), base_url, format }
    }
}
...
impl DataProviderInterface for MoodleClient {
    ...
    async fn get_courses(&self, token: &str, user_id: i64) -> Result<Vec<Course>, Error> {
        let url = format!("{}wstoken={}&wsfunction=core_enrol_get_users_courses{}&userid={}",
                          self.base_url,
                          token, 
                          self.format,
                          user_id,
    ...
    async fn get_grades_by_course_id(&self, token: &str, user_id: i64, course_id: i64) -> Result<UserGrades, Error> {
        let url = format!("{}wstoken={}&wsfunction=gradereport_user_get_grade_items{}&userid={}&courseid={}",
            self.base_url,
            token,
            self.format,
            user_id,
    ...
    async fn get_deadline_by_course_id(&self, token: &str, course_id: i64) -> Result<Events, Error> {
        let url = format!("{}wstoken={}&wsfunction=core_calendar_get_action_events_by_course{}&courseid={}",
            self.base_url,
            token,
            self.format,
            course_id,
    ...
    async fn get_grades_overview(&self, token: &str) -> Result<GradesOverview, Error> {
        let url = format!("{}wstoken={}&wsfunction=gradereport_overview_get_course_grades{}",
            self.base_url,
            token,
            self.format
        );
